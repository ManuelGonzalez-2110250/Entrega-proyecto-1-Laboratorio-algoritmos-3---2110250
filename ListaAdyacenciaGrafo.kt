class ListaAdyacenciaGrafo<T>: Grafo<T> {
    ...
    override fun conectar(desde: T, hasta: T): Boolean{
    	if(contiene(desde) && contiene(hasta)){
    	    var nodoAct: NodoVer<T>? = head
    	    while (nodoAct != null && nodoAct.ver != desde){
    	        nodoAct = nodoAct.nodoVerSig
    	    }
    	    var nodoLado: NodoLad<T>? = nodoAct?.nodoLadAsoc
    	    if(nodoAct!!.nodoLadAsoc == null){
    	        var nodoLadNuevo: NodoLad<T> = NodoLad(v = hasta, siguiente = null)
    	        nodoAct.nodoLadAsoc = nodoLadNuevo
    	        return true
    	    }
    	    while (nodoLado?.siguiente != null){
    	    	//si consigue ya una conexión no puede conectarlo de nuevo
    	    	if (nodoLado.v == hasta) return false
    	    	nodoLado = nodoLado.siguiente
    	    }
    	    //solo buscará hasta n-1 por el que sea igual, por lo que se añade en n el if
    	    if (nodoLado!!.v == hasta) return false
    	    var nodoLadNuevo: NodoLad<T> = NodoLad(v = hasta, siguiente = null)
    	    nodoLado.siguiente = nodoLadNuevo
    	    //si no se detiene retorna true
    	    return true
    	}
    	//si no consigue ninguno de los dos vertices retorna false
    	else return false
    }
